...
**Answer (2c)** There are multiple ways to create this model.

  - Include all `main effects` and all required `interactions`
  
```{r}
largeHospModel = lm(Blood ~ Age + Education +  Sex + Income + Sex : Age + Sex : Income, data = hospitalData)
largeHospModel
```
...
```
  ...
  - All coefficients of both these models are equal
  
  - Note that predictions from both models are also same.
  
```{r}
all.equal(predict(largeHospModel), predict(shortHospModel))
```
...
```
  ...
  **Exploration for finding easier way to get coefficients**

  Another potentially easier way would be if we could problematically get coefficients for Rate (i.e. $\beta_3$, $\beta_5$, $\beta_6$ and $\beta_7$)

  I found this while exploring the model. `days_int$terms` provides this information. This object has attribute "factors" which contains name of coefficients affecting slope of any variable.
    
  `attr(days_int$terms, "factors")["Rate", ]` will have value as 1 for indexes of all coefficients that impact slope of Rate.
    
```{r}
# we do this be removing intercept from the coefficients because terms object does not contain that.
days_int$coefficients[-1][attr(days_int$terms, "factors")["Rate", ] == 1]
```
...
**Answer (3c)**
  ...
  - It's same as **2b** but for additive model this time.
